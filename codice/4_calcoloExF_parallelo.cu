...
/*
* funzione che calcola il grado dei cluster: ogni cluster ? formato da nodo seme e altri due nodi (in questo caso non importa se i nodi sono a distanza 1 o a distanza 2)
...
__device__ int gradoCluster(int* nodi, int* IR, int* IC) {
	...
	for (int a = 0; a < 3; a++) {
		...
		if (valRiga > 0) {//controllo se il numero di vicini ? >0 altrimenti il grado ? 0
			...
			for (int k = IR[nodo - 1] + 1; k < IR[nodo - 1] + 1 + valRiga; k++) {//scandisco tutti i vicini del nodo considerato e controllo se questi nodi non sono gi? nel cluster
				...
				if (vicino != nodi[0] && vicino != nodi[1] && vicino != nodi[2]) {// se il vicino del nodo non ? nel cluster aumento il grado del cluster
					grado++;
				}
...
/*
 * funzione che viene invocata dal kernel expectedForce: ogni thread calcola il grado dei cluster composti da (seme,distOne[thread],distOne[j]) con distOne[j] che varia
*/
__global__ void clusterDistOne(int seed, int* distOne, int* IR_vec, int* IC_vec, int* gradi, int* indiceGradi, int* totalFI, int indiceDistOne)
{
	//indice k per scorrere l'array IR_vec 
	int k = blockIdx.x * blockDim.x + threadIdx.x;


	if (k < indiceDistOne) {//controllo di non eccedere la dimensione dell'array distOne
		for (int j = k + 1; j < indiceDistOne; j++) {//scorro la parte dell'array distOne a destra della posizione k
			...
			//gradi[indiceGradi[0] + 2 * k] = grado;
			//gradi[indiceGradi[0] + (2 * k + 1)] = grado;
			//ogni cluster è presente in due combinazioni
			gradi[indiceGradi[0] + 2 * k] = grado;
			gradi[indiceGradi[0] + 2 * k + 1] = grado;

			atomicAdd(&totalFI[0], mult * grado);//sommo a totalFI mult * grado
			atomicAdd(&indiceGradi[0], 2);//aumento il valore di indiceGradi di 2 perchè ogni ciclo iterazione del ciclo for calcola 2 combinazioni
...
/*
 * funzione che viene invocata dal kernel expectedForce: ogni thread calcola il grado dei cluster composti da (seme,distOne[thread],distOne[j]) con distOne[j] che varia tra i vicini di distOne[thread]
*/
__global__ void clusterDistOneTwo(int seed, int* distOne, int* IR_vec, int* IC_vec, int* gradi, int* indiceGradi, int* totalFI, int indiceDistOne)
{
	//indice k per scorrere l'array IR_vec 
	int k = blockIdx.x * blockDim.x + threadIdx.x;
	...
	if (k < indiceDistOne) {//controllo di non eccedere la dimensione dell'array distOne
		...
		if (valRiga > 0) { //se il nodo considerato ha almeno un vicino posso procedere con il calcolo del cluster
			...
			for (int j = IR_vec[nodo - 1] + 1; j < IR_vec[nodo - 1] + 1 + valRiga; j++) { //scandisco i vicini del nodo considerato che sono i nodi a distanza 2 del nodo seed
				...
				int index = atomicAdd(&indiceGradi[0], 1);
				//gradi[indiceGradi[0]+k*valRiga] = grado; //inserisco il grado calcolato nel vettore gradi
				gradi[index] = grado;

				atomicAdd(&totalFI[0], grado);


				//atomicAdd(&offset[0], 1);


			}

			//atomicAdd(&indiceGradi[0], offset[0]);
		}
...
/*
*funzione che esegue reduce a livello di warp
*/
...
/*
* child grid che viene chiamata da ogni thread del parent kernel: normalizza il vettore ed esegue la riduzione
*/
__global__ void calcolExf(double* ExF, int* gradi, int* indiceGradi, int* totalFI) {
	double sum = double(0);
	//double norm = double(0);
	for (int i = blockDim.x * blockIdx.x + threadIdx.x; i < indiceGradi[0]; i += blockDim.x * gridDim.x)
	{
		if (gradi[i] != 0) {

			//norm= (double)gradi[i] / totalFI[0];
			sum += (double)gradi[i] / totalFI[0];
		}
	}
...
*/
__global__ void expectedForce(int* IR_vec, int* IC_vec, int n_IR, int gradoMax, double* __restrict__ d_exf)
{

	// indice seed per scorrere l'array IR_vec 
	...
	if (seed < n_IR && seed != 0) {//controllo di non eccedere la dimensione dell'array e che non venga calcolato ExF del primo elemento di IR_vec
		double* ExF = new double[1];
		ExF[0] = 0; //ogni thread memorizza il valore di ExF in questa variabile

		int* indiceGradi = new int[1];//vettore di 1 elemento per calcolare la lunghezza dell'array gradi
		indiceGradi[0] = 0;
		int* totalFI = new int[1];//vettore di 1 elemento per calcolare totalFI di ogni nodo
		totalFI[0] = 0;
		...
		//printf("valore in posizione 0 di distOne:%i", distOne[gradoMax]);
		...
		else // se il nodo considerato ha almeno un vicino
		{
			...
			if (indiceDistOne == 0) {//se a questo punto il vettore distOne è vuoto allora il nodo seme punta solo a se stesso
				...
				//calcolo la dimensione dell'array gradi considerando il caso peggiore: ogni nodo nodo ha gradoMax vicini e tutti i suoi vicini hanno gradoMax vicini
				...
				//ogni thread lancia la child grid clusterDistOne
				int threadsPerBlock = 32;
				int blocksPerGrid = (indiceDistOne + threadsPerBlock - 1) / threadsPerBlock;
				clusterDistOne << <blocksPerGrid, threadsPerBlock >> > (seed, distOne, IR_vec, IC_vec, gradi, indiceGradi, totalFI, indiceDistOne);
				cudaDeviceSynchronize();
				cudaError_t err = cudaGetLastError();
				if (err != cudaSuccess) printf("Errore:%ld\n", err);

				//ogni thread lancia la child grid clusterDistOneTwo
				clusterDistOneTwo << <blocksPerGrid, threadsPerBlock >> > (seed, distOne, IR_vec, IC_vec, gradi, indiceGradi, totalFI, indiceDistOne);
				cudaDeviceSynchronize();
				cudaError_t err2 = cudaGetLastError();
				if (err2 != cudaSuccess) printf("Errore:%ld\n", err2);
			...
			* - se la somma totale dei gradi ? 0 allora anche ExF=0
			...
			*/
				...
				if (totalFI[0] == 0) {
					...
					int threadsPerBlock = 32;
					int blocksPerGrid = (indiceGradi[0] + threadsPerBlock - 1) / threadsPerBlock;

					calcolExf << <blocksPerGrid, threadsPerBlock >> > (ExF, gradi, indiceGradi, totalFI);
					cudaDeviceSynchronize();
					cudaError_t err = cudaGetLastError();
					if (err != cudaSuccess) printf("Errore:%ld\n", err);
					d_exf[seed] = ExF[0];
...
int main(int argc, char* argv[])
{
	...
	cudaDeviceSetLimit(cudaLimitMallocHeapSize, malloc_limit); //imposto la capienza della memoria heap a mallo_limit
	...
	double* d_exf;
	cudaMalloc(&d_exf, size);
	cudaMemcpy(d_exf, h_exf, size, cudaMemcpyHostToDevice);
	...
	cudaMemcpy(h_exf, d_exf, size, cudaMemcpyDeviceToHost);
