		...
		else // se il nodo considerato ha almeno un vicino
		{
			...
			*/
				...
				if (totalFI[0] == 0) {
					...
					double norm = 0;
					//printf("indice gradi per il nodo seed %i:%i\n", seed, indiceGradi[0]);
					for (int K = 0; K < indiceGradi[0]; K++) { //normalizzo l'array gradi e calcolo il valore di ExF
						if (gradi[K] != 0) {

							//printf("grado per il nodo%i:%i\n", seed, gradi[K]);

						  //printf("totalFI per il nodo%i:%i\n", seed, totalFI[0]);
							norm = (float)gradi[K] / totalFI[0];
							ExF -= log(norm) * norm;
						}
